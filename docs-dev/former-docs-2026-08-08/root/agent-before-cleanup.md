...
4. 消息历史恢复：sessions API 已提供消息存储，前端按会话恢复仍可完善。
5. 自动保存：手动编辑后同步后端。
...
- LangGraph 已实现：`app/agent/graph.py` StateGraph + `app/agent/nodes/` 19 个节点，`ws_agent.py` 中 `precision_mode=true` 走 LangGraph 精密模式；旧文档写"Phase 3 待开始 / graph.py 预留"已过时。
- 会话 REST API 已实现：`app/api/sessions.py`（GET/POST/PUT/DELETE `/api/sessions` + `/{session_id}/messages`），旧文档写"会话管理预留"已过时。
- 组合构件工具层已存在：`app/tools/component_tools.py`（组件级 validate/fix，被 base_component_node 等节点调用），旧文档工具清单未包含。
