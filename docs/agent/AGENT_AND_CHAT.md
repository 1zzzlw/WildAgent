...
模型服务错误和建筑校验错误必须分开处理。`403 Free quota exhausted` 表示模型账号额度耗尽，不代表门、窗或屋顶参数有问题。组件生成节点会把供应商异常转换为统一错误类别和中文提示；合并节点发现任一组件模型调用失败后，会停止本次生成，不保存缺少组件的半成品，也不进入 callback。额度与鉴权问题需要先处理账号或模型配置；限流、超时等临时问题需要稍后由用户重新发起生成。
