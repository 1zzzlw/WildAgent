...
- [ ] 先创建协议类型，不先写 Shader；
- [ ] 后端 validator 拒绝越界值和 Shader 字符串；
- [ ] Core 原样传递规范化 `procedural`；
- [ ] renderer 新建独立的 `proceduralMaterials` 目录；
- [ ] `materialSignature` 包含程序化参数；
- [ ] `customProgramCacheKey` 只包含 Shader 结构，不包含所有数值；
- [ ] 完成砖格和砂浆后立即跑渲染测试；
- [ ] 再增加程序化法线；
- [ ] 最后增加风化、盐碱和流痕；
- [ ] 用直墙、带洞墙、曲墙和多层模板做视觉验收；
- [ ] 运行 `npm run check:core`、`npm run check:rendering` 和 `npm run build`；
- [ ] 后端运行相关材质 Schema 测试；
- [ ] 把协议补充进 `WILD_BLUEPRINT_SPEC.md`，把测试说明补充进 `TESTING.md`。
