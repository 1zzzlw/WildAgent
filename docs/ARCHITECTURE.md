...
本轮已经补齐协议版本、结构化步骤、统一保存门禁、Turn 服务端恢复、图级路由测试、“结构化错误 → 模型选择工具 → 程序执行 → 复检提交/回滚”的定向修复闭环，以及骨架空间不变量、组件修复复检和 Core 重建诊断。当前主要技术债转为：快速/精密模式仍有意图分类与事件适配重复、固定质量评测集仍需加入更多真实 `.wild`，以及会话元数据、Turn、消息和 `.wild` 尚未形成事务。Phase 3A 人工验收完成前不进入 PBR 素材协议和存储改造，具体门禁见 [优化路线](ROADMAP.md)。
