...
// 直接内嵌天坛蓝图，构建后成为自包含单文件，双击打开即渲染（不依赖 /bp 服务）。
import tiantanRaw from '../tiantan.wild?raw'
...
function frameToBox(box: { min: number[]; max: number[] }): void {
  ...
  // 足迹居中归一化（生成器产出的是角点对齐坐标，天坛本身已是中心对齐，平移≈0）。
...
async function load(): Promise<void> {
  const loading = document.getElementById('loading') as HTMLElement
  try {
    const bp = parseWildBlueprint(tiantanRaw)
    const entity = await reconstructWildEntity(bp)
    instance = new BlueprintRenderInstance('tiantan-single')
