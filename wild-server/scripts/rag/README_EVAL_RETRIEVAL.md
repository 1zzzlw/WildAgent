...
| 指标 | 单题如何计算 | 总体如何计算 | 主要回答的问题 |
|---|---|---|---|
| `Hit@K` | Top-K 至少命中一个 `expectedSources` 为 1，否则为 0 | 所有可评分问题的平均值 | 有没有碰到至少一个正确来源 |
| `Recall@K` | 命中的标准来源数 ÷ 标准来源总数 | 每道题 Recall 的宏平均 | 应该找到的来源找全了多少 |
| `MRR@K` | 第一条正确结果排名的倒数；未命中为 0 | 所有可评分问题倒数排名的平均值 | 第一条正确知识排得有多靠前 |

例如一道题有两个标准来源，Top-3 只在第 2 名找到其中一个，则 `Hit@3=1`、`Recall@3=1/2=0.5`、单题倒数排名为 `1/2=0.5`。

`Hit@K` 不反映是否找全，`Recall@K` 不惩罚混入的错误结果，`MRR@K` 只关心第一条正确结果。因此三项必须一起看，并结合逐题命中明细判断噪声。完整入门说明和更多例子见 [README.md 的“三个核心指标怎么理解”](README.md#4-三个核心指标怎么理解)。
