...
| `test_architecture_plan.py` | 架构规划：总体体量、立面和骨架生成约束 |
...
| 文件 | 用例 | 覆盖点 | 运行命令 |
|---|---|---|---|
| `test_architecture_plan.py` | 45 | 架构规划核心：最小请求保持单墙、旧房间布局载荷隔离、退台体量楼梯连续、幕墙密集立面、高层示意、洞口/入口附件、楼层数中文语义与公共建筑轮廓排序 | `python -m pytest tests/components/test_architecture_plan.py -v` |
