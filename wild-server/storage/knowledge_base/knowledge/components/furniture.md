...
🔴 **`rotation` 只有"弧度三维数组"一种合法写法**，务必写成
`[0, 弧度值, 0]`（只转向就只动第 2 个分量）：

| 写法 | 结果 |
|---|---|
| `"rotation": [0, 1.5708, 0]` | ✅ 正确（90°） |
| `"rotation": 90` / `180` | ⚠️ 度数**标量**：引擎会自动按"绕 Y 的度数"换算成 `[0, 1.5708, 0]`，不会丢件，但属于不合契约的写法 |
| `"rotation": [0, 90, 0]` | ⚠️ 度数数组：分量超出 2π 时按度数自动换算 |
| `"rotation": [0, 7.0, 0]` | ❌ 既超出 2π、又不是 15 的整数倍 → 认不出单位，**校验会报错** |

换算只是**兜底**：写对了不会触发任何修正，写错了也不会因此整批报废
（曾经就是度数标量让 8 个家具片段全被判死 → 最终一件家具都没有）。
弧度换算：`90° = 1.5708`、`180° = 3.1416`、`270° = 4.7124`。
