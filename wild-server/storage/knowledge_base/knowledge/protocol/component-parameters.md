...
**严禁使用的值**：`pitched`、`sloped`、`gabled`、`hipped`、`shed`、`mono-pitch` 等。
这些是有效的建筑学术语，但**不是** WILD 的枚举值，写进去必然编译失败。

**取尺寸与定位规则**：

- `span` / `depth` 按其负责的**体量轮廓**取尺寸；出檐时在两侧增加相应余量。
  **不得**简单用整栋建筑的外包络当作屋顶范围。
