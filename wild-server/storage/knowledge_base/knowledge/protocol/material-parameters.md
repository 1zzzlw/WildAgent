...
entity_type: schema
entity_name: material_parameters
topic: parameters
wild_version: "1.1"
status: supported
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `baseColor` | [R, G, B] | 是 | 在标准光照 D65 正午下的 sRGB 基础色。范围 0.0–1.0 |
...
所有 WILD 数值颜色（包括 `baseColor`、`emissive`、效果层颜色和程序化顶点色）都按 sRGB authored value 表达。渲染器必须在光照计算前将它们转换到线性工作空间，输出时再编码为 sRGB；不得把数值数组直接当作线性颜色使用。纹理通道遵循同一原则：基础色纹理按 sRGB 解码，法线、粗糙度、金属度和 AO 作为非颜色数据读取。

所有合规客户端在渲染时必须先使用标准光照模型（PBR 或 NPR）将实时光照（如黄昏暖光）计算为偏移量，再叠加到 `baseColor` 上。这样确保了标准光照下所有客户端渲染一致，而实际光照下各客户端按各自风格自动调整。

### 1.3 格式铁律

- `baseColor` **必须是 `[R, G, B]` 数组**（每个分量 0.0–1.0）。
  **绝对禁止**写成 `"#RRGGBB"` 字符串 —— 这是最常犯的格式错误。
- 用户未指定颜色时，由本次材质方案决定，**不继承建筑类型的默认配色**。
- 不要默认把墙、楼板和屋顶全部设成同一个 `concrete`；墙、楼板、屋顶、门、玻璃应按实际角色引用各自的材质名。
- **物理玻璃必须写全**：`materialClass: "glass"`、`transmission > 0`、`ior`；
  `opacity` 必须为 `1` 或直接省略。缺 `materialClass` 的"玻璃"只会渲染成一块不透明板。

| 材质角色 | 关键要求 |
|----------|----------|
| 玻璃 | `materialClass: "glass"`、`side: "double"`、`transmission > 0`（如 0.92）、`ior`（如 1.5）、`thickness`、`opacity: 1` 或省略 |
| 金属 | `metallic` 接近 1.0，`roughness` 偏低 |
| 木材 / 石材 / 瓦 | `metallic: 0.0`，靠 `roughness` 与 `baseColor` 区分 |

**引用完整性**：

- 墙、楼板、框、门扇、屋面和玻璃各自引用本次材质方案中的材料 ID。
  相同设计角色可跨节点复用同一 ID；不同物理角色需要不同渲染参数时使用不同 ID。
- 所有 `material`、`frameMaterial`、`leafMaterial`、`glassMaterial` 引用**必须存在于蓝图 `materials`
  或受支持的全局材质库**，否则校验直接失败。
- 材料名只是 ID —— 名字叫 `concrete` 不证明它是混凝土，也不能用来声称真实结构或专业性能。
- 玻璃与不透明框架必须使用**不同**的材料引用。
...
除下文已有的风化、苔藓和边缘磨损外，v1.1 正式增加 `grain`：

| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `type` | string | 是 | 固定值 `"grain"` |
| `intensity` | number | 是 | 顶点颜色扰动强度，建议 0–1 |
| `scale` | number | 是 | 程序化噪声空间尺度 |

`grain` 在当前参考引擎中是确定性的程序化顶点色变化，适合木材、皮革、橡胶等低成本表面层次；它不是法线贴图的替代品。
...
基础数值材质、`embeddedImage` 及原有效果层保持 v1.0 语义；`textures`、`normalScale`、`uvScale` 和 `grain` 为 v1.1 增量能力。v1.0 材质继续有效。
