...
entity_type: schema
entity_name: behavior_instructions
topic: parameters
wild_version: "1.1"
status: supported
...
三类动态指令各自的作用对象不同，不能互相替代：

| 字段 | 作用范围 | 说明 |
|---|---|---|
| `behaviors.physics` | 整个实体的物理存在 | 对 `type: "building"`，质量设为 0 表示整体不可移动；约束定义构件间的机械连接关系 |
| `behaviors.animation` | 化身实体的运动姿态 | **仅对 `type: "avatar"` 有效**，对建筑无意义 |
| `behaviors.scripts` | 由事件条件中的构件 id 定位到的具体元素 | 见下文交互脚本 |
