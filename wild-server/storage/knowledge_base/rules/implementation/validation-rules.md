...
### 4.4 楼梯与楼板：楼梯井必须用多块楼板拼出

**规则**：`stair` 上方必须有洞口让人通过。`floor` 是实心矩形、**没有开洞字段**，
所以楼梯井不能写成"带洞楼板"，必须把该层楼板拆成**多块矩形 floor 围出井口**：

- 井口水平范围 ≥ 楼梯轮廓 + 两侧各 ≥0.05m，且覆盖**跑步上半段**（人头部高出板底的位置）。
- 楼梯两端端点（中线与两侧 ±width/2 共 3 个采样点）必须**落在井口之外的楼板上**——
  校验器 `collect_stair_placement_issues` 按这 3 个点判"起点/终点是否在标高匹配的楼板范围内"，
  端点落进井里会报 `stair_end_outside_floor`。
- 校验器**不会**检查"楼梯上方是否被楼板压住"（element 间碰撞只查同类型），
  楼板盖住楼梯不报错，靠本规则在生成侧约束。

以下是 `geometry.elements` 数组中的一组片段，不是完整 `.wild` 文件（材质等字段省略）：

```json
[
  { "type": "stair", "id": "stair_main", "from": [4.2, 0.0, 8.6], "to": [7.0, 3.0, 8.6], "width": 1.1 },
  { "type": "floor", "id": "floor_l2_s", "shape": "rect", "from": [0.0, 2.8, 0.0], "to": [14.0, 2.8, 8.0], "thickness": 0.2 },
  { "type": "floor", "id": "floor_l2_n", "shape": "rect", "from": [0.0, 2.8, 9.2], "to": [14.0, 2.8, 10.0], "thickness": 0.2 },
  { "type": "floor", "id": "floor_l2_w", "shape": "rect", "from": [0.0, 2.8, 8.0], "to": [5.0, 2.8, 9.2], "thickness": 0.2 },
  { "type": "floor", "id": "floor_l2_e", "shape": "rect", "from": [6.9, 2.8, 8.0], "to": [14.0, 2.8, 9.2], "thickness": 0.2 }
]
```

上例洞口为 x 5.0–6.9、z 8.0–9.2（楼梯宽 1.1 → 轮廓 z 8.05–9.15，两侧各留 0.05m 缝），
楼梯 `to.x = 7.0` 延伸到东块楼板上，保证两端采样点全部落在楼板内。
